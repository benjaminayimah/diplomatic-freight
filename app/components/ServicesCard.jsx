...
function ServicesCard({ service, onClick, isOpen, id }) {
  ...
  return (
    <div className='w-full md:w-[382px] shrink-0'>
      <div onClick={onClick} className="group relative aspect-3/4 rounded-3xl w-full overflow-hidden cursor-pointer">
        ...
        <div className="absolute flex flex-col justify-end p-4 inset-0 z-5">
          <div className='flex justify-end'>
            <button onClick={onClick}
              className={`
                ${isOpen && id === service.id ? 'rotate-45' : ''}
                h-10 w-10 rounded-3xl grid place-items-center
