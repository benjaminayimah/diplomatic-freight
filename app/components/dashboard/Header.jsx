...
function Header() {
  ...
  const UserAvatar = (
    <button className={`${menuOpen ? 'bg-gray-100 text-black' : 'text-gray-700'} flex py-0.5 pl-0.5 pr-2.5 gap-2 items-center hover:text-black rounded-3xl transition duration-300 border border-gray-100 hover:bg-gray-50`}>
      ...
      <div className="flex gap-1 items-center max-w-25">
        <span className="font-base whitespace-nowrap truncate">
          {user?.name}
        </span>
        <ChevronDownIcon strokeWidth={2} className="h-3.5 shrink-0" />
      </div>
    </button>
  );
