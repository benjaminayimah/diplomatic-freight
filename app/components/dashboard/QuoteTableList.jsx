      ...
      <div className='p-5 border border-gray-200 hover:bg-gray-50 transition-colors duration-300 inline-block w-full rounded-[10px]'
        >
          <div className='flex justify-between'>
            <div>
              <Link href={`/app/quote/${quote?.id}`} className='font-medium group-hover:underline'><span>{quote?.name}</span></Link>
              <div className='text-sm text-gray-500'><span className='font-medium'>Date created: </span><span>{dateFormat(quote?.createdAt)}</span></div>
            </div>
            <div>
